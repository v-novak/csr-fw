...
OUTPUT_CODE = 'hub/adjustment.cpp'
...
def precalc(alpha_min=ALPHA_MIN, alpha_max=ALPHA_MAX, alpha_step=ALPHA_STEP):
    ...
    return [(int(math.degrees(calc_beta(math.radians(alpha)))), rpm2(rpm1, math.radians(alpha)))
            for alpha in range(alpha_min, alpha_max + alpha_step, alpha_step)]
        #print('{},{:.2f},{},{:.2f}'.format(alpha, )))

def main():
    code_template = Template("""
    // This code was generated by $script, do not commit it
    ...
    code = code_template.substitute(script=sys.argv[0],
                                    values=values_str,
                                    alpha_min=ALPHA_MIN,
